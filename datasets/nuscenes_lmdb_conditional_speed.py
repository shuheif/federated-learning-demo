...
class ImageDataset(Dataset):
    def __init__(self, 
        ...
        for my_scene in dataset[_set]:
            ...
            while True:
                ...
                cmd_path = dataset_path+'commands/'+cam_front_data['filename'].split('/')[-1]+'.npy'
                # print(cmd_path)
                # print(os.path.exists(cmd_path))
                if not os.path.exists((cmd_path)):
                    # continue
                    break
                # else:
                #     future_x,future_y = np.load(cmd_path) 

                # all_episode_data.append([first_sample, cam_front_data, speed])
                all_episode_data.append([first_sample, cam_front_data, speed, cmd_path])
    ...
    def __getitem__(self, idx):
        ...
        sample, cam_front_data, speed, cmd_path = all_episode_data[index]
        ...
        if rad >=0 and  rad < (math.pi*55/180):
            cmd = 6
        elif rad >= (math.pi*55/180) and rad < (math.pi*70/180):
            cmd = 5
            ...
            cmd = 2
        elif rad >= (math.pi*110/180) and rad < (math.pi*125/180):
            cmd = 1
        elif rad >= (math.pi*125/180) and rad <= (math.pi*180/180):
            cmd = 0
        ...
        for j in range(1,6):
            ind = index + j
            next_sample, next_cam_front_data, next_speed, next_cmd = all_episode_data[ind]
        ...
        return rgb_images, np.array(locations), cmd, speed
