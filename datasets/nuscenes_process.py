...
# nusc_can = NuScenesCanBus(dataroot='/data/jimuyang/lbwyt/nuscenes-mini')

nusc = NuScenes(version='v1.0-trainval', dataroot='/data/jimuyang/lbwyt/nuscenes', verbose=True)
# nusc = NuScenes(version='v1.0-mini', dataroot='/data/jimuyang/lbwyt/nuscenes-mini', verbose=True)

# save_root = Path('/data/jimuyang/lbwyt/visualization/')
# save_root = Path('/data/jimuyang/lbwyt/visualization_1/')

# if not os.path.exists(str((save_root))):
# 	(save_root).mkdir()

# out = cv2.VideoWriter('/data/jimuyang/lbwyt/output_all.mp4',cv2.VideoWriter_fourcc(*'MP4V'), 5, (1600,900))
# out = cv2.VideoWriter('/data/jimuyang/lbwyt/output_all.mp4',cv2.VideoWriter_fourcc(*'MP4V'), 5, (800,450))
out = cv2.VideoWriter('/data/jimuyang/lbwyt/output_vis.mp4',cv2.VideoWriter_fourcc(*'MP4V'), 5, (800,450))
...
# my_scene = nusc.scene[0]
	...
	for i in range(length-5):
		...
		# image = cv2.imread('/data/jimuyang/lbwyt/nuscenes-mini/' + cam_front_data['filename'])
		image = cv2.imread('/data/jimuyang/lbwyt/nuscenes/' + cam_front_data['filename'])
	...
	# print the camera pose!!! It should be obvious from the ego pose? Or should be fixed from ego vehicle?
			...
			# cords_y_minus_z_x = np.concatenate([-cords_x_y_z[0, :], cords_x_y_z[1, :], cords_x_y_z[2, :]]) # this is almost right!!!
			# cords_y_minus_z_x = np.concatenate([-cords_x_y_z[0, :], cords_x_y_z[2, :], cords_x_y_z[1, :]]) # bad
			# cords_y_minus_z_x = np.concatenate([cords_x_y_z[1, :], -cords_x_y_z[0, :], cords_x_y_z[2, :]]) # bad
			# cords_y_minus_z_x = np.concatenate([cords_x_y_z[1, :], cords_x_y_z[2, :], -cords_x_y_z[0, :]]) # bad
			# cords_y_minus_z_x = np.concatenate([cords_x_y_z[0, :], -cords_x_y_z[1, :], cords_x_y_z[2, :]]) # bad

			# cords_y_minus_z_x = np.concatenate([cords_x_y_z[0, :], cords_x_y_z[1, :], cords_x_y_z[2, :]]) # good!
