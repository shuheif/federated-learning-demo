...
from datasets.federated_dataset_jt.baidu_data2 import ApolloDataset


class FederatedDataset():
    def __init__(self, av2_cities=None, ns_cities=None, wm_cities=None, baidu_cities=None, av2_root="/data/shared/av2_all/train",
                 ns_train_proportion=0.8, ns_train_test="TRAIN", waymo_train_test="TRAIN", waymo_p=1, av2_p=1, baidu_train_test = 'TRAIN'):
        ...
        if baidu_cities is not None:
            if baidu_train_test== 'TRAIN':
                self.set_baidu()
                # self.set_waymo(cities=wm_cities, proportion=0.05)
                print("FederatedDataset: There are", len(self.baidu_dataset), " ns samples overall. ")
            else:
                self.set_baidu(dataset_dir='/home/ruizhao/data/applo_mini_test')
                # self.set_waymo(cities=wm_cities, tfrecords_dir="/data/shared/waymo_test", proportion=0.05,
                #                images_dir="/home/data/waymo_test")
        else:
            self.baidu_dataset = None
    ...
    def set_baidu(self, dataset_dir="/home/data/apollo_train", proportion=1.0):
        '''
        ...
        self.baidu_dataset = ApolloDataset(dataset_dir=dataset_dir, default_cityid=11, proportion=proportion)
    ...
    def __len__(self):
        ...
        if self.baidu_dataset is not None:
            length += len(self.baidu_dataset)
    ...
    def __getitem__(self, index):
        ...
        if self.baidu_dataset is not None:
            if index < len(self.baidu_dataset):
                return self.baidu_dataset[index]
            else:
                index = index - len(self.baidu_dataset)


if __name__ == "__main__":
    datasets = FederatedDataset(av2_cities=['PIT', 'WDC', 'MIA', 'ATX', 'PAO', 'DTW'], ns_cities=['BOS', 'SGP'],
                                av2_root="/data/shared/av2_all/mini_test")
