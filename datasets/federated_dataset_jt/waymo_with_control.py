WAYMO_DATASET_JT_VERSION = "v0.3 beta"
...
class WaymoSample():
    ...
    def get_image_numpy_bgr(self, frame=None):
        if frame is None:
            # tfrecord = tf.data.TFRecordDataset(self.tfrecord_path, compression_type='')
            tfrecord = get_tfrecord(self.tfrecord_path)
            # data = list(tfrecord)[self.data_index]
            # frame = open_dataset.Frame()
            # frame.ParseFromString(bytearray(data.numpy()))
            frame = get_frame(tfrecord, self.data_index)
        front_camera_image = frame.images[0]  # 第一张 Image 是正前方方向的。

        image_numpy_bgr = cv2.imdecode(np.fromstring(front_camera_image.image, dtype=np.uint8), cv2.IMREAD_UNCHANGED)

        # image_tf=tf.image.decode_jpeg(front_camera_image.image)
        # image_numpy_rgb=image_tf.numpy()
        # image_numpy_bgr = cv2.cvtColor(image_numpy_rgb, cv2.COLOR_RGB2BGR)
...
class WaymoDataset():
    def __init__(self, cities, tfrecords_dir, images_dir, proportion=1,cityid_map=None):
        print("Waymo Dataset", WAYMO_DATASET_JT_VERSION)
        random.seed(9999)
        ...
        for entry in os.listdir(path):
            ...
            ifprevious_first = False
            for index, sample in enumerate(log_all_samples):
                if sample.previous_timestamp_nanosec is None:
                    first_timestamp = sample.timestamp_nanosec
                    first_location = sample.transform
                    second_timestamp = sample.next_timestamp_nanosec
                    second_location = sample.next_transform

                    sample.velocity = (second_location.translation - first_location.translation) / (
                            (int(second_timestamp) - int(first_timestamp)) * 1e-9)
                    sample.velocity = np.linalg.norm(sample.velocity)

                    ifprevious_first = True

                elif sample.next_timestamp_nanosec is None:
                    first_timestamp = sample.previous_timestamp_nanosec
                    first_location = sample.previous_transform
                    second_timestamp = sample.timestamp_nanosec
                    second_location = sample.transform

                    sample.velocity = (second_location.translation - first_location.translation) / (
                            (int(second_timestamp) - int(first_timestamp)) * 1e-9)
                    sample.velocity = np.linalg.norm(sample.velocity)

                    previous_sample = log_all_samples[index-1]
                    sample.acceleration = previous_sample.acceleration
                    sample.brake = previous_sample.brake
                    sample.throttle = previous_sample.throttle
                    ifprevious_first = False
                    ...
                    first_velocity = (second_location.translation - sample.transform.translation) / (
                            (int(second_timestamp) - int(sample.timestamp_nanosec)) * 1e-9)
                    first_velocity = np.linalg.norm(first_velocity)
                    second_velocity = (sample.transform.translation - first_location.translation) / (
                            (int(sample.timestamp_nanosec) - int(first_timestamp)) * 1e-9)
                    second_velocity = np.linalg.norm(second_velocity)
                    acceleration = (first_velocity - second_velocity)/ (
                            (int(second_timestamp) -  int(first_timestamp)) * 1e-9 / 2)


                    if acceleration>0:
                        if acceleration>6:
                            acceleration = 6
                        sample.throttle = acceleration/6.5
                        sample.brake = 0
                    else:
                        if acceleration<-6:
                            acceleration=-6
                        sample.throttle = 0
                        sample.brake = acceleration / (-6.5)


                    sample.acceleration = acceleration
                    # sample.acceleration = np.linalg.norm(sample.acceleration)
                    if ifprevious_first:
                        previous_sample = log_all_samples[index-1]
                        previous_sample.acceleration = sample.acceleration
                        previous_sample.brake  = sample.brake
                        previous_sample.throttle = sample.throttle
                    ifprevious_first = False
    ...
    def __getitem__(self, index):
        ...
        control = np.array([sample.brake, sample.throttle, sample.radian])
        return sample.get_image_tensor_bgr_from_disk(), str(
            sample.image_path), sample.trajectory, sample.command, sample.velocity, self.cityid_map[sample.city], control


if __name__ == '__main__':
    test_data = WaymoDataset(cities=['location_other'],proportion=0.2,tfrecords_dir="/data/shared/waymo_val",
