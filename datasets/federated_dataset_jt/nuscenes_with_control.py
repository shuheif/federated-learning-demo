...
NUSCENES_DATASET_JT_VERSION = "v1.2"
...
from .nuscenes_jt.scene_indices import *
# from nuscenes_jt.scene_indices import *
...
class NuScenesDataset(Dataset):
    def __init__(self, cities=None, random_seed=99, train_proportion=1.0, train_or_test=TRAIN, cityid_map=None):
        ...
        for scene in useful_scenes:
            ...
            while True:
                ...
                sample_index_within_scene += 1
                cam_front_data = NUSCENES.get('sample_data', sample['data']['CAM_FRONT'])
                anno_path = self.dataset_path + 'annotations/' + cam_front_data['filename'].split('/')[-1].replace(
                    'jpg',
                    'npy')
                ...
                # ego_pose_token = NUSCENES.get('sample_data', sample['data']['CAM_FRONT'])['ego_pose_token']
                ...
                # coords = []
                # sizes = []
                # # translate all the object in world space
                # for anno_token in sample['anns']:
                #     obj = NUSCENES.get('sample_annotation', anno_token)
                ...
                # jtsample.object_info = [coords, sizes]
            ...
            self.jtsamples.extend(scene_jtsamples[
                                  :-gap])  # this makes sure I do not use last 'gap' frames as the training data (just for wpts generation)
    ...
    def __getitem__(self, idx):
        ...
        control =np.array([brake/126, throttle/1000, (steer+780)/1560])
        ...
        for i in range(5):
            ...
            # object_info = self.jtsamples[idx+i].object_info
        ...
        return bgr_image, bgr_name, np.array(locations_vehicle), cmd, speed, cityid, control

if __name__ == "__main__":
    cities = ['BOS']
    nuscenes_dataset=NuScenesDataset(cities=cities,random_seed=10,
                                              train_or_test="TRAIN",
