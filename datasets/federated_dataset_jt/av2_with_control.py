...
PICKLE_FOLDER_DIR = '/data/jt_data_space/feddrive_project/pickles2/'  # jtcheckpoint pickles2
...
class AV2Sample():
    ...
    def get_image_tensor(self, crop_and_resize):
        ...
        if self.can_put_to_memory():
            ...
            # save it to the disk.
        # 将 image_tensor 缓存至硬盘对速度的有一定提升，但不是特别大，但对硬盘空间的占用非常大，有爆炸的风险。
        # 所以不是正式 train 的时候，可将其开启。正式 train 的时候，建议将其关闭，防止意外发生。
...
class AV2Dataset():
    ...
    def __init__(self,
                 dataset_dir="/data/shared/av2_all/train",
                 cities=[],
                 proportion=1,
        ...
        for log_id in log_ids:
            ...
            for index, sample in enumerate(log_all_samples):
                if sample.previous_timestamp is None:
                    ...
                    first_velocity = (second_location.translation - sample.location.translation) / (
                            (int(second_timestamp) - int(sample.timestamp)) * 1e-9)
                    first_velocity = np.linalg.norm(first_velocity)
                    second_velocity = (sample.location.translation - first_location.translation) / (
                            (int(sample.timestamp) - int(first_timestamp)) * 1e-9)
    ...
    def __getitem__(self, index):
        ...
        return sample.get_image_tensor(self.crop_and_resize), str(
            sample.image_path), sample.trajectory, sample.command, sample.velocity, cityid, control

    # def get_sample(self,index=None,timestamp=None):
...
print("Module av2_dataset_jttry2:", MODULE_VERSION, "imported.")
...
# Test list
# sample points
# different dataset_dir
if __name__ == '__main__':
    test_data = AV2Dataset(cities = ['PAO'],dataset_dir="/data/shared/av2_all/train")
    alist = []
    blist = []
    for i in range(807):
        alist.append(test_data[i][6])
    print('haha')
