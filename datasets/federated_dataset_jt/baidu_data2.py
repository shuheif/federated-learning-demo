...
class ApolloSample:
    def __init__(self) -> None:
        ...
        self.velocity = None
        self.radian = None  # 这是错误的
        self.trajectory = None  # 错误的
        self.command = None  # 这是错误的
        self.rpyxyz = None
        self.radian_2 = None  # 这是错误的
        self.command_2 = None  # 这是错误的
        self.radian_3 = None  # 正确的
        self.trajectory_3 = None  # 正确的
        self.command_3 = None  # 正确的
        self.cityid = 11
...
class ApolloDataset:
    def __init__(self, dataset_dir="/home/data/apollo_train",default_cityid =11, proportion=1) -> None:
        '''
        @param dataset_dir
        ...
        for road_folder in os.listdir(dataset_path):
            ...
            for record_folder in os.listdir(records_dir_path):
                ...
                self.fill_command_and_trajectory_3(record_all_samples=record_all_samples)

                no_use_index = -1
                for index in range(len(record_all_samples)):
                    if record_all_samples[index].command_3 is None or \
                            record_all_samples[index].radian_3 is None or \
                            record_all_samples[index].trajectory_3 is None:
                        no_use_index = index
                        break
                ...
                print("record all samples number", len(record_all_samples))
                print("num samples added", len(record_all_samples[:no_use_index]))
                self.samples.extend(record_all_samples[:no_use_index])  # todo
        self.samples = random.sample(self.samples,k= int(proportion * len(self.samples)))

    def fill_command_and_trajectory_3(self, record_all_samples):
        ...
        for index, sample in enumerate(record_all_samples):
            ...
            yaw = sample.rpyxyz[2]
            rotation = np.array([[cos(-yaw), -sin(-yaw)],
                                 [sin(-yaw), cos(-yaw)]
                                 ])

            origin_rotated_xy = np.matmul(rotation, sample.se3[0:2, 3])
            relative_xy_list = []
            for i in range(5):
                xy = self.interpolate_se3(list_se3, list_timestamp_nanosec,
                                          sample.timestamp_nanosec + (i + 1) * 0.5 * 1e9)[0:2, 3]

                rotated_xy = np.matmul(rotation, xy)

                relative_xy = rotated_xy - origin_rotated_xy
                relative_xy_list.append(relative_xy)

            # if index==105:
            #     print(relative_xy_list)

            trajectory = np.array(relative_xy_list)
            ...
            sample.radian_3 = radian
            sample.trajectory_3 = trajectory
            sample.command_3 = command
    ...
    def interpolate_se3(self, list_se3, list_timestamp_nanosec, timestamp_nanosec):
        ...
        for row in range(3):
            for col in range(4):
                f = interp1d(list_timestamp_nanosec, list_se3[:, row, col])
                result[row, col] = f(timestamp_nanosec)
    ...
    def __getitem__(self, index):
        ...
        return sample.get_tensor_bgr(), str(sample.image_path), sample.trajectory_3, sample.command_3, sample.velocity, self.default_cityid
        # sample.cityid
