...
class AV2Sample():
    ...
    def get_wgs84(self):  # latitude longitude
        xy = np.array(self.location.translation[0:2])
        return convert_city_coords_to_wgs84(np.array([xy]), self.city_name)[0]
    def get_ctiyid(self):
        xy = self.get_wgs84()
        if self.city_name == 'PIT':
            if xy[0]< 40.4475:
                cityid = 0 # pit downtown
            else:
                cityid = 1 # pit not downtown
        elif self.city_name == 'WDC':
            if xy[0]< -38.895:
                cityid = 2
            elif xy[1]< -76.99:
                cityid = 3 # wdc downtown
            else:
                cityid = 4 # wdc not downtown
        elif self.city_name =='MIA':
            if xy[1]<-80.17:
                if xy[0]>25.78:
                    cityid = 5 #miami downtown
                else:
                    cityid = 6 #miami downtown
            else:
                if xy[0]>25.78:
                    cityid = 7 #miami downtown
                else:
                    cityid = 8 #miami beach
        elif self.city_name == 'ATX':
            cityid = 9  # ATX
        elif self.city_name == 'PAO':
            cityid = 10  # PAO
        elif self.city_name == 'DTW':
            if xy[1]>-83.10:
                cityid = 11 #downtown
            else:
                cityid = 12 # not downtown
...
class AV2Dataset():
    ...
    def __getitem__(self, index):
        ...
        return sample.get_image_tensor(self.crop_and_resize), str(
            sample.image_path), sample.trajectory, sample.command, sample.velocity, sample.get_ctiyid()# sample.get_wgs84()
